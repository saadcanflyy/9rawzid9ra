...
- **`resolve_academic_context()` faculty resolution bug (found 2026-09-26, Prompt 25 testing, not yet fixed)**: `entity_aliases()` extracts a faculty's *entire* parenthesised short name verbatim as one alias string — e.g. "École Normale Supérieure de Rabat (ENS Rabat)" contributes the bare word `rabat` as a standalone matchable alias, not just `ens`/`ensr`. Since the faculty-matching `ORDER BY length(alias) DESC` treats every matched alias equally, a query like `"fs rabat"` (meant as **F**aculté des **S**ciences + Rabat) can resolve to *any* Rabat-based faculty whose parenthetical happens to contain "Rabat" (e.g. ENS Rabat, ENSAM Rabat) instead of Faculté des Sciences de Rabat (FSR), because the incidental 5-letter alias `rabat` outranks FSR's legitimate 2-letter alias `fs`. Reproduced live: `select resolve_academic_context('analyse 2 fs rabat examen')` returns `faculty: "École Normale Supérieure de Rabat"`, not FSR. This is not an `institution_aliases` gap (no insert fixes it) — it needs either stripping place-words from the literal parenthetical extraction in `entity_aliases()`, or excluding place-words from the faculty-alias match candidates (universities can keep city-name matching; faculties sharing a city shouldn't). Not fixed here — flagged for a dedicated, tested change.
